...
interface TubeMapMotionEngine {
    fun observe(snapshot: TubeMapSnapshot): TubeMapSnapshot
    fun advance(snapshot: TubeMapSnapshot, currentTime: Instant): TubeMapSnapshot
}

class RealTubeMapMotionEngine : TubeMapMotionEngine {
    ...
    override fun observe(snapshot: TubeMapSnapshot): TubeMapSnapshot {
        val lineIndex = snapshot.lines.associateBy(TubeLine::id)
        val activeTrainIds = snapshot.trains.map(TubeMapTrain::trainId).toSet()
    ...
    override fun advance(snapshot: TubeMapSnapshot, currentTime: Instant): TubeMapSnapshot {
        val projectedLines = snapshot.lines.associateBy(TubeLine::id) { line -> ProjectedTubeLine(line) }
    ...
    private fun advanceTrain(
        train: TubeMapTrain,
        projectedLine: ProjectedTubeLine?,
        currentTime: Instant
    ): TubeMapTrain {
